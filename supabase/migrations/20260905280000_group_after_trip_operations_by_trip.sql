-- Presentation-only extension: include lifecycle status in the existing Admin read model.
CREATE OR REPLACE FUNCTION public.get_after_trip_operations(_booking_id uuid DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER STABLE SET search_path=public AS $$
DECLARE bookings_json jsonb; sources_json jsonb;
BEGIN
  IF NOT public.current_profile_is_admin() THEN RAISE EXCEPTION 'Authoritative Admin required.'; END IF;
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'charge_id',c.id,'booking_id',c.booking_id,'reservation_number',b.reservation_number,'trip_status',b.trip_status,
    'vehicle',concat_ws(' ',v.brand,v.model),'guest_name',r.full_name,'guest_email',r.email,'host_name',h.full_name,'host_email',h.email,
    'category',c.category,'amount_cents',c.amount_cents,'currency',c.currency,'explanation',c.explanation,'charge_status',c.status,'submitted_at',c.submitted_at,
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object('ledger_entry_id',l.id,'booking_id',l.booking_id,'source_type',CASE WHEN l.effect='deposit_capture' THEN 'security_deposit' ELSE 'separate_payment' END,'description',l.description,'external_reference',l.external_reference,'amount_cents',l.amount_cents,'refunded_cents',COALESCE(ref.refunded,0),'allocated_cents',COALESCE(used.allocated,0),'available_cents',GREATEST(0,l.amount_cents-COALESCE(ref.refunded,0)-COALESCE(used.allocated,0)),'currency',l.currency,'occurred_at',l.occurred_at) ORDER BY l.occurred_at),'[]'::jsonb) INTO sources_json
  ...
  WHERE (_booking_id IS NULL OR l.booking_id=_booking_id) AND l.status='succeeded' AND l.external_reference IS NOT NULL AND(l.effect='deposit_capture' OR(l.effect='payment' AND l.category='after_trip_payment'));
