...
import { FinancialReconciliationPanel } from "@/components/admin/FinancialReconciliationPanel";
import { HistoricalAfterTripReconciliation } from "@/components/admin/HistoricalAfterTripReconciliation";
...
type DepositAttempt = { payment_intent_id: string | null; created: number | null; original_authorization: number | null; currency: string | null; customer_id: string | null; current_status: string | null; current_capturable: number | null; cancellation_timestamp: number | null; cancellation_reason: string | null; cancellation_origin: string; charge_id: string | null; settled: boolean; association: { confidence: "CONFIRMED" | "PROBABLE" | "AMBIGUOUS" | "REJECTED"; reasons: string[] }; history: DepositHistory };
type DepositAttemptSummary = { total_candidates_discovered: number; confirmed_attempts: number; probable_attempts: number; ambiguous_attempts: number; rejected_candidates: number; total_actually_captured: number | null; total_refunded: number; net_deposit_retained: number | null; final_confirmed_outcome: string };
type Snapshot = {
  ...
  security_deposit_attempts: { summary: DepositAttemptSummary; timeline: DepositAttempt[] };
...
export function FinancialInspectorTab() {
  ...
  return <div className="space-y-6">
    ...
    {snapshot && <div className="space-y-6">
      ...
      <Card><CardHeader><CardTitle>Security Deposit Attempts</CardTitle><CardDescription>Authorization attempts are shown separately and are not summed as revenue. Only CONFIRMED attempts contribute to proven captured/refunded totals.</CardDescription></CardHeader><CardContent className="space-y-4"><div className="grid gap-4 sm:grid-cols-3"><Value label="Candidates discovered">{snapshot.security_deposit_attempts.summary.total_candidates_discovered}</Value><Value label="Confirmed attempts">{snapshot.security_deposit_attempts.summary.confirmed_attempts}</Value><Value label="Probable / ambiguous">{snapshot.security_deposit_attempts.summary.probable_attempts + snapshot.security_deposit_attempts.summary.ambiguous_attempts}</Value><Value label="Total actually captured">{formatInspectorMoney(snapshot.security_deposit_attempts.summary.total_actually_captured)}</Value><Value label="Total refunded">{formatInspectorMoney(snapshot.security_deposit_attempts.summary.total_refunded)}</Value><Value label="Net deposit retained">{formatInspectorMoney(snapshot.security_deposit_attempts.summary.net_deposit_retained)}</Value></div><div className="space-y-3">{snapshot.security_deposit_attempts.timeline.map((attempt) => <div key={attempt.payment_intent_id || String(attempt.created)} className="rounded-lg border border-border p-4"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><p className="font-semibold">{formatStripeTimestamp(attempt.created)} — {formatInspectorMoney(attempt.original_authorization, attempt.currency || "usd")} authorization</p><Badge variant={attempt.association.confidence === "CONFIRMED" ? "default" : "secondary"}>{attempt.association.confidence}</Badge></div><div className="grid gap-3 sm:grid-cols-3"><Value label="PaymentIntent">{attempt.payment_intent_id}</Value><Value label="Current status">{attempt.current_status}</Value><Value label="Cancellation origin">{attempt.cancellation_origin.replace(/_/g, " ")}</Value><Value label="Captured (proven)">{formatInspectorMoney(attempt.history.historical_captured_amount, attempt.currency || "usd")}</Value><Value label="Released (proven)">{formatInspectorMoney(attempt.history.historical_released_uncaptured_amount, attempt.currency || "usd")}</Value><Value label="Refunded">{formatInspectorMoney(attempt.history.historical_refunded_amount, attempt.currency || "usd")}</Value><Value label="Current capturable">{formatInspectorMoney(attempt.current_capturable, attempt.currency || "usd")}</Value><Value label="Association evidence">{attempt.association.reasons.join(", ").replace(/_/g, " ")}</Value></div></div>)}{snapshot.security_deposit_attempts.timeline.length === 0 && <p className="text-sm text-muted-foreground">No security-deposit attempts were discovered for this booking.</p>}</div></CardContent></Card>
      ...
      <FinancialReconciliationPanel bookingId={snapshot.booking.booking_id} snapshot={snapshot} />
      <HistoricalAfterTripReconciliation bookingId={snapshot.booking.booking_id} />
    </div>}
  </div>;
}