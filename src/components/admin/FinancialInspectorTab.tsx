...
type Snapshot = {
  ...
  security_deposit: null | { payment_intent_id: string | null; stripe_status: string | null; original_authorization: number | null; amount_received: number | null; captured_amount: number | null; amount_capturable: number | null; released_or_uncaptured_amount: number | null; settled: boolean; canceled_at: number | null; created: number | null; latest_charge: StripeCharge | null };
...
export function FinancialInspectorTab() {
  ...
  return <div className="space-y-6">
    ...
    {snapshot && <div className="space-y-6">
      ...
      <Card><CardHeader><CardTitle>Security Deposit</CardTitle></CardHeader><CardContent className="space-y-4">{snapshot.security_deposit ? <><div className="grid gap-4 sm:grid-cols-3"><Value label="PaymentIntent">{snapshot.security_deposit.payment_intent_id}</Value><Value label="Stripe status">{snapshot.security_deposit.stripe_status}</Value><Value label="Settled">{snapshot.security_deposit.settled ? "Yes" : "No"}</Value><Value label="Authorized">{formatInspectorMoney(snapshot.security_deposit.original_authorization)}</Value><Value label="Captured">{formatInspectorMoney(snapshot.security_deposit.captured_amount)}</Value><Value label="Remaining capturable">{formatInspectorMoney(snapshot.security_deposit.amount_capturable)}</Value><Value label="Released / uncaptured">{formatInspectorMoney(snapshot.security_deposit.released_or_uncaptured_amount)}</Value><Value label="Created">{formatStripeTimestamp(snapshot.security_deposit.created)}</Value></div><ChargeDetails charge={snapshot.security_deposit.latest_charge} /></> : <p className="text-sm text-muted-foreground">No security-deposit PaymentIntent is associated with this booking.</p>}</CardContent></Card>
